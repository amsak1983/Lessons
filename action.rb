class Action < Menu
  ...
  def attach_carriage
    ...
    carriage = PassengerCarriage.new if train.type.eql?(:passenger)
    carriage = CargoCarriage.new if train.type.eql?(:cargo)
    count.times { train.attach_carriage(carriage) }
