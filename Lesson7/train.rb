class Train
  ...
  def with_each_carriages(&_block)
  ...
  def carriage_valid?(carriage)
    ...
    raise "Не подходящий тип вагона!" unless carriage.type.eql?(carriage.type)
    true
  end

  def validate!
    raise "Неправильный номер поезда. Повторите ввод." if self.number !~ NUMBER_FORMAT
  end
end
